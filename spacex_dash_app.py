...
def get_scatter_chart(entered_site,payload):
    ...
    payload_df1 = spacex_df[(spacex_df['Payload Mass (kg)'] > low) & (spacex_df['Payload Mass (kg)'] < high)]
    if entered_site == 'ALL':
        print('INSIDE ALL')
        fig = px.scatter(payload_df1, x="Payload Mass (kg)", 
        y="class", color="Booster Version Category", 
        title=f"Correlation between Payload and Success for all Sites")
        return fig
    else:
        print("inside ELSE")
        payload_df2= payload_df1[payload_df1 ['Launch Site'] == entered_site]
        fig = px.scatter(payload_df2, x= "Payload Mass (kg)", 
        y="class",color="Booster Version Category",
